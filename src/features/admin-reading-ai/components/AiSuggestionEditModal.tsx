...
interface AiSuggestionEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (patch: Partial<AiVocabularySuggestion>) => Promise<void>;
  suggestion: AiVocabularySuggestion | null;
  isSaving: boolean;
}
...
export function AiSuggestionEditModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm transition-all duration-300">
      <div className="bg-white w-full max-w-lg rounded-3xl border border-slate-100 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden transform scale-100 animate-in fade-in zoom-in-95 duration-250">
        
        {/* Header */}
        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-50">
          <div>
            <h3 className="text-xl font-black text-text-primary">Edit Suggestion</h3>
            <p className="text-xs text-text-secondary mt-0.5">Refine the AI vocabulary suggestion details.</p>
          </div>
          <button
            onClick={onClose}
